...
"""Main inference engine for axiomcuda - coordinates all models in the CUDA backend."""
...
import axiomcuda.planner as mppi


def init(key, config, observation, action_dim):
    """Initialize all models for the CUDA backend.
...
def step_fn(
    ...
    """Main inference step for CUDA backend.
...
def plan_fn(key, carry, config, action_dim):
    """Planning wrapper for CUDA backend.
...
def reduce_fn_rmm(key, rmm_model, cxm=None, dxm=None, n_samples=2000, n_pairs=2000):
    """BMR (Bayesian Model Reduction) for RMM.
