...
def forward_default(x):
    """Default forward dynamics for TMM.
    
    Args:
        x: State vector.
        
    Returns:
        Predicted next state using default dynamics.
    """
    x_tmm = x[:, :6]
...
def _in_ellipse(x, y, cx, cy, rx, ry):
    """Check if point (x, y) is inside ellipse centered at (cx, cy) with radii (rx, ry).
    ...
    Returns:
        Boolean indicating if point is inside ellipse.
    """
...
def _object_interactions(x_t, i, j, n=3, r=1, return_grid=False):
    """Compute interaction between two objects.
    ...
    Args:
        ...
        n: Grid resolution.
        r: Radius factor.
        return_grid: Whether to return the overlap grid.
        
    Returns:
        Tuple of (is_interacting, distance) and optionally grid info.
    """
    ...
    return (is_interacting, distance)
...
def get_interacting_objects_ellipse(
    ...
    # pick the most interesting interacting object
...
def interacting_squares(x1, y1, w1, h1, x2, y2, w2, h2):
    """Check if two squares are interacting.
    
    Args:
        x1, y1, w1, h1: First object position and size.
        x2, y2, w2, h2: Second object position and size.
        
    Returns:
        Boolean indicating if objects are interacting.
    """
...
def get_distance(x_t, i, j, o=0.1):
    ...
    Args:
        x_t: Object states.
        i, j: Object indices.
        o: Offset for interaction.
        
    Returns:
        Tuple of (interacting, absolute_distance, relative_distance).
    """
    cx_i, cy_i, w_i, h_i = x_t[i, [0, 1, 6, 7]]
    cx_j, cy_j, w_j, h_j = x_t[j, [0, 1, 6, 7]]
...
def rollout_sample(
    ...
    **kwargs,
    ...
    Args:
        ...
        x_input: Initial state.
        actions: Action sequence.
        tracked_obj_ids: Tracked object IDs.
        num_features: Number of additional features.
        num_samples: Number of samples.
        position_noise: Noise for position.
        velocity_noise: Noise for velocity.
        interact_with_static: Whether to interact with static objects.
        r_interacting_predict: Radius factor for prediction.
        num_switches: Number of TMM switches.
        r_ell_threshold: Threshold for reward model.
        forward_predict: Whether to forward predict.
        reward_prob_threshold: Threshold for reward prediction.
...
def rollout(
    ...
    **kwargs,
    ...
    Args:
        ...
        r_ell_threshold: Threshold for reward model.
        num_switches: Number of TMM switches.
        object_identities: Object identities.
        r_interacting_predict: Radius factor for prediction.
        forward_predict: Whether to forward predict.
        reward_prob_threshold: Threshold for reward prediction.
        stable_r: Use stable radius.
    ...
    Returns:
        Tuple of (predictions, tmm_slots, rmm_slots, info_gains, predicted_rewards).
    """
    if key is not None:
    ...
    def step_fn(x_t, input):
        ...
        # Use reward from most salient object
...
def _find_pairs(key, rmm, n_samples=None):
    """Find pairs of similar components for merging.
    ...
    Returns:
        Array of pairs to consider for merging.
    """
...
def consider_merge(c_data, d_data, elbo_before, mixture, used_mask, idx_1, idx_2):
    """Consider merging two components.
    
    Args:
        c_data: Continuous data.
        d_data: Discrete data.
        elbo_before: Current ELBO.
        mixture: Mixture model.
        used_mask: Used component mask.
        idx_1, idx_2: Indices of components to merge.
        
    Returns:
        Tuple of (mixture, used_mask, elbo, merged).
    """
...
def compute_elbo(mm, c_data, d_data):
    ...
    and the reward, and only considers the prior complexities.
...
def run_bmr(key, rmm, n_samples, pairs=None, cxm=None, dxm=None):
    ...
    Args:
        ...
        pairs: Pairs to consider for merging.
        cxm: Continuous means.
        dxm: Discrete means.
