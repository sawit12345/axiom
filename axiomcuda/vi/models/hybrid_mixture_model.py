...
from axiomcuda.vi import ArrayDict
from axiomcuda.vi.conjugate import Multinomial, MultivariateNormal


class HybridMixture(eqx.Module):
    """Mixture model with discrete and continuous likelihoods - CUDA accelerated."""
    ...
    def get_sample_dims(self, data, batch_dim, lh_event_dim):
        sample_dims = tuple(range(len(data.shape) - batch_dim - self.prior.event_dim - lh_event_dim))
        return sample_dims
    ...
    def _e_step(self, c_data: Array, d_data: List[Array], w_disc=None):
        ...
        c_data = self.expand_to_categorical_dims(c_data, self.continuous_likelihood.batch_dim, self.continuous_likelihood.event_dim)
        ...
        for i, dlh in enumerate(self.discrete_likelihoods):
            d_data_i = self.expand_to_categorical_dims(d_data[i], dlh.batch_dim, dlh.event_dim)
    ...
    def _m_step(self, c_data: Array, d_data: List[Array], qz: Array):
        ...
        c_data = self.expand_to_categorical_dims(c_data, self.continuous_likelihood.batch_dim, self.continuous_likelihood.event_dim)
        sample_dims = self.get_sample_dims(c_data, self.prior.batch_dim, self.continuous_likelihood.event_dim)
        self.prior.update_from_statistics(self._to_stats(qz, sample_dims), **self.pi_opts)
    ...
    def _sum_components(self, other, self_mask, other_mask):
        ...
        self.prior.posterior_params = jtu.tree_map(fn, other.prior.prior_params, self.prior.posterior_params, other.prior.posterior_params)
        self.continuous_likelihood.posterior_params = jtu.tree_map(fn, other.continuous_likelihood.prior_params, self.continuous_likelihood.posterior_params, other.continuous_likelihood.posterior_params)

        for i in range(len(self.discrete_likelihoods)):
            self.discrete_likelihoods[i].posterior_params = jtu.tree_map(fn, other.discrete_likelihoods[i].prior_params, self.discrete_likelihoods[i].posterior_params, other.discrete_likelihoods[i].posterior_params)

    def _combine_params(self, other, select_from_other_mask):
        def fn(x_0, x_1):
            u = select_from_other_mask.reshape(-1, *[1 for _ in range(len(x_0.shape) - 1)])
            return x_1 * u + x_0 * (1 - u)

        self.prior.posterior_params = jtu.tree_map(lambda x_0, x_1: fn(x_0, x_1), self.prior.posterior_params, other.prior.posterior_params)
        self.continuous_likelihood.posterior_params = jtu.tree_map(lambda x_0, x_1: fn(x_0, x_1), self.continuous_likelihood.posterior_params, other.continuous_likelihood.posterior_params)

        for i in range(len(self.discrete_likelihoods)):
            self.discrete_likelihoods[i].posterior_params = jtu.tree_map(lambda x_0, x_1: fn(x_0, x_1), self.discrete_likelihoods[i].posterior_params, other.discrete_likelihoods[i].posterior_params)

    def _merge_clusters(self, idx_1, idx_2):
        def combine_fn(prior, posterior):
            posterior = posterior.at[idx_1].set(posterior[idx_1] + posterior[idx_2] - prior[idx_2])
            posterior = posterior.at[idx_2].set(prior[idx_2])
            return posterior

        self.prior.posterior_params = jtu.tree_map(combine_fn, self.prior.prior_params, self.prior.posterior_params)
        self.continuous_likelihood.posterior_params = jtu.tree_map(combine_fn, self.continuous_likelihood.prior_params, self.continuous_likelihood.posterior_params)
    ...
    def sample(self, key, n_samples=1):
        ...
        def make_pd(mat):
            ...
            mat = mat + jnp.broadcast_to(jnp.eye(mat.shape[-1]), mat.shape) * 2 * (eps - min_eig) * (min_eig < 0)
            return mat
        ...
        def _sample(_key):
            ...
            c_sample = jr.multivariate_normal(_subkeys[1], mu[z, :, 0], si[z])[..., None]
            d_sample = [_sample_discrete(_subkeys[2 + i], self.discrete_likelihoods[i].alpha[z, :, 0], to_1h=True)[..., None] for i in range(len(self.discrete_likelihoods))]
