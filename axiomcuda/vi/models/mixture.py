...
from axiomcuda.vi import ArrayDict, Conjugate, Distribution
from axiomcuda.vi.models import Model
from ..utils import assign_unused


class Mixture(Model):
    """
    Mixture model - CUDA accelerated.
    
    Wraps C++ CUDA kernels for EM algorithm operations.
    """
    ...
    def __init__(
        ...
        self.assign_unused_opts = assign_unused_opts if assign_unused_opts is not None else {"d_alpha_thr": 1.0, "fill_value": 1.0}
    ...
    def assign_unused(
        ...
        d_alpha_r = (self.prior.alpha - self.prior.prior_alpha).reshape(self.flattened_batch_shape + self.event_shape_prior)
    ...
    def get_assignments_from_data(self, data: Union[Array, Tuple]) -> Array:
        data = self.expand_to_categorical_dims(data)
        return softmax(self.likelihood.expected_log_likelihood(data) + self.prior.log_mean(), list(range(-self.prior.event_dim, 0)))

    def get_assignments_from_probabilities(self, inputs: Union[Distribution, Tuple[Distribution]]) -> Array:
        inputs = self.expand_to_categorical_dims_for_probs(inputs)
        return softmax(self.likelihood.average_energy(inputs) + self.prior.log_mean(), list(range(-self.prior.event_dim, 0)))
